...
void zb_update_hum(int humidity)
{
    ...
    {
        ESP_LOGW(TAG_ZB_UPDATE_HUM, "Device is not connected! Could not measure the humidity level");
    }
}
...
void zb_report_hum()
{
    ...
    {
        ESP_LOGW(TAG_ZB_UPDATE_HUM, "Device is not connected! Could not report the humidity level");
    }
}